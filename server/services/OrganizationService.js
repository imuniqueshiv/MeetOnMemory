...
const joinOrganization = async (userId, organization, invitationToken = null) => {
  ...
  // Enforce visibility & invitation checks for non-public organizations
  ...
  if (orgVisibility !== "public") {
    ...
    if (!invitation) {
      ...
      // Throw exact error messages expected by tests
      if (orgVisibility === "invite-only" || organization.joinPolicy === "invite_only") {
        throw new ForbiddenError(
          "This organization requires a valid invitation to join.",
        );
      } else {
    ...
    // For public org, if there's a pending request, throw conflict
    if (pendingRequest) {
      throw new ConflictError(
        "A membership request for this organization is already pending.",
      );
 ...
 */
export const createOrJoinOrganization = async (userId, orgName, invitationToken = null) => {
  ...
  if (organization) {
    ...
    const joinResult = await joinOrganization(userId, organization, invitationToken);
 ...
 */
export const joinOrganizationById = async (userId, organizationId, invitationToken = null) => {
  if (!organizationId) {
  ...
  await joinOrganization(userId, organization, invitationToken);
