    ...
    const decryptData = async () => {
      if (needsDecryption && encryptionKey && typeof transcript === 'string' && isEncrypted(transcript)) {
        try {
          ...
          let parsedDec;
          try {
            parsedDec = JSON.parse(dec); // Try to parse as JSON if it was an object
          } catch(e) {
            parsedDec = dec;
          }
          if (parsedDec && parsedDec.segments) {
             parsedDec.segments = parsedDec.segments.filter(
              (segment, index, self) =>
                index ===
                self.findIndex(
                  (s) =>
