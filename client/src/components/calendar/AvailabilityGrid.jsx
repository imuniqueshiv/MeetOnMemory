  ...
  if (!freeBusyData || Object.keys(freeBusyData).length === 0) {
    return (
      <div className="availability-grid-empty">
        ...
        <p className="empty-hint">Try selecting a different date range or ensure all participants have connected calendars.</p>
      </div>
    );
  }
      ...
      <div className="availability-grid">
        ...
        {Object.entries(conflictMap).map(([key, slot]) => {
          ...
          return (
            <div 
              key={key}
              className={`grid-row ${isConflict ? 'conflict-row' : ''} ${isBusy ? 'busy-row' : ''}`}
              onClick={() => {
            ...
            >
              ...
              {participants.map((participant, idx) => (
                <div key={idx} className="participant-column">
                  <div className={`availability-indicator ${slot.participants[participant.email] ? 'busy' : 'free'}`}>
